...
export default {
  ...
  addVisitorServiceAgreement: asyncHandler(async (req, res) => {
    ...
    // Get visitor with all required data
    const visitor = await visitorService.getVisitorById(visitorId);
    if (!visitor) {
      throw { status: NOTFOUNDCODE, message: "Visitor not found" };
    }
    ...
    if (!visitor.details) {
      throw {
        status: BADREQUESTCODE,
        message:
          "Visitor details not found. Please complete your information first.",
      };
    }
    ...
    if (!visitor.estimate) {
      throw {
        status: BADREQUESTCODE,
        message: "Estimate not found. Please complete the estimate step first.",
      };
    }

    // Check if agreement already exists
    const existingAgreement = await db.visitorServiceAgreement.findUnique({
      where: { visitorId },
    });

    if (existingAgreement) {
      // Check if visitor is ready for project creation
      const { visitorConversionService } = await import(
        "../../services/visitorConversionService"
      );
      const projectReadiness =
        await visitorConversionService.canConvertVisitor(visitorId);

      httpResponse(req, res, SUCCESSCODE, "Agreement already accepted", {
        documentUrl: existingAgreement.documentUrl,
        acceptedAt: existingAgreement.acceptedAt,
        agreementVersion: existingAgreement.agreementVersion,
        message:
          "Agreement already exists. Download from the URL or check your email.",
        // Project readiness info
        projectReady: projectReadiness.canConvert,
        projectReadyReason: projectReadiness.reason || null,
      });
      return;
    }
    ...
    const agreementData: ClientServiceAgreementData = {
      recipientName: visitor.details.fullName,
      recipientEmail: visitor.details.businessEmail,
      estimatedPriceMin: Number(visitor.estimate.estimateFinalPriceMin),
      estimatedPriceMax: Number(visitor.estimate.estimateFinalPriceMax),
      ipAddress,
      userAgent,
      acceptedAt: new Date(),
    };
    ...
    await db.visitorServiceAgreement.create({
      data: {
        visitorId,
    ...
    httpResponse(
      ...
      res,
      SUCCESSCODE,
      "Service agreement accepted successfully. PDF generated and sent to your email.",
      {
        documentUrl: result.documentUrl,
        agreementVersion: result.agreementVersion,
        ...
        message:
          "You can now proceed to register as a client and secure your project.",
        // Project readiness info
        projectReady: projectReadiness.canConvert,
        projectReadyReason: projectReadiness.reason || null,
      },
