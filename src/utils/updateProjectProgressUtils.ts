...
export const updateProjectProgress = async (
  ...
  const totalProgressPoints = milestones.reduce(
    (sum, milestone) => sum + (milestone.totalProgressPoints || 0),
    0,
  );
 ...
 */
export const distributeMilestonePoints = (
  milestones: Milestone[],
): Milestone[] => {
  if (!milestones || milestones.length === 0) return milestones;

  // Get sum of all priority ranks
  const totalPriorities = milestones.reduce(
    (sum, milestone) => sum + (milestone.priorityRank || 1),
    0,
  );

  if (totalPriorities <= 0)
    throwError(BADREQUESTCODE, "Invalid priority ranks");
