...
} from "../../utils/updateProjectProgressUtils";
...
export default {
  createSingleProjectMilestone: asyncHandler(async (req, res) => {
    ...
    if (!existingMilestones)
      throwError(BADREQUESTCODE, "Failed to fetch existing milestones");
    ...
    const allMilestones = [
      ...existingMilestones,
      { ...milestoneData, projectId: Number(projectId) },
    ];
    ...
    const newTotalPoints =
      updatedMilestones[newMilestoneIndex]?.totalProgressPoints || 100;
  ...
  createMultipleMileStones: asyncHandler(async (req, res) => {
    ...
    if (!milestoneData || milestoneData.length === 0)
      throwError(BADREQUESTCODE, "No milestone data provided");

    const mileStoneNames = milestoneData
      .map((milestone) => milestone.mileStoneName)
      .filter(Boolean);
    if (mileStoneNames.length === 0)
      throwError(BADREQUESTCODE, "Invalid milestone names");
    ...
    if (existingMilestones && existingMilestones.length > 0)
      throwError(BADREQUESTCODE, "Milestone already exists");
    ...
    const newMilestonesWithPoints = milestoneData.map((milestone, index) => {
      ...
      const points =
        updatedMilestones[updatedIndex]?.totalProgressPoints ||
    ...
    httpResponse(
      ...
      res,
      CREATEDCODE,
      "Project Milestones created successfully",
    );
  }),
  ...
  updateMileStone: asyncHandler(async (req, res) => {
    ...
    if (
      milestoneData.priorityRank !== undefined &&
      milestoneData.priorityRank !== milestone?.priorityRank &&
      milestone?.projectId
    ) {
      const projectMilestones = await db.milestone.findMany({
  ...
  deleteMileStone: asyncHandler(async (req, res) => {
    ...
    if (!projectId)
      throwError(BADREQUESTCODE, "Invalid project ID in milestone");
  ...
  updateMilestoneProgress: asyncHandler(async (req, res) => {
    ...
    if (progress === undefined || progress === null)
      throwError(BADREQUESTCODE, "Progress value is required");
    ...
    if (progress < 0 || progress > milestone?.totalProgressPoints) {
      throwError(
        BADREQUESTCODE,
        `Progress must be between 0 and ${milestone?.totalProgressPoints}`,
      );
    }
