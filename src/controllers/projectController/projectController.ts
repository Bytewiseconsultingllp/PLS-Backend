...
export default {
  createProject: asyncHandler(async (req, res) => {
    ...
    if (!clientWhoPostedThisProjectForeignIdExists)
      throw { status: BADREQUESTCODE, message: "Client id does not exist." };
    const isProjectAlreadyExist = await db.project.findUnique({
    ...
    const createdProject = await db.project.create({
      data: {
        ...
        clientWhoPostedThisProjectForeignId:
          projectData.clientWhoPostedThisProjectForeignId,
      },
      select: {
        id: true,
        title: true,
  ...
  getSingleProject: asyncHandler(async (req, res) => {
    ...
    if (!project)
      throw { status: BADREQUESTCODE, message: "Project not found." };
