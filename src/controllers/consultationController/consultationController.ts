...
export default {
  ...
  createConsultation: asyncHandler(async (req, res) => {
    const { name, email, phone, message, address, subject } =
      req.body as TCONSULTATION;
    ...
    const isConsultationDateAlreadyBooked =
      await db.consultationBooking.findUnique({
        where: {
  ...
  updateConsultation: asyncHandler(async (req, res) => {
    ...
    const isConsultationDateAlreadyBooked =
      await db.consultationBooking.findFirst({
        where: {
          bookingDate,
          id: { not: Number(id) },
        },
  ...
  getSingleRequestedConsultation: asyncHandler(async (req, res) => {
    ...
    if (!consultation)
      throw { status: NOTFOUNDCODE, message: "No consultation found" };
    httpResponse(req, res, SUCCESSCODE, "Single consultation", consultation);
  }),
  // ** delete consultation
  deleteRequestedConsultation: asyncHandler(async (req, res) => {
    const { id } = req.params;
    const checkIfConsultationExists = await db.consultationBooking.findUnique({
      where: { id: Number(id) },
    });
  ...
  rejectConsultationBooking: asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { rejectAndDeleteConsultaion = true } = req.body as {
      rejectAndDeleteConsultaion: boolean;
    };
    if (!id)
      throw {
        status: BADREQUESTCODE,
