...
export default {
  ...
  createHireUsRequest: asyncHandler(async (req, res) => {
    ...
    const responseAfterUploadingFilesOnCloudinary = uploadResponses.map(
      (response) => ({
    ...
    await gloabalMailMessage(
      data.email,
      HIREUSMESSAGE,
      "Prime Logic Solution",
      "Request to hire Prime Logic Solution",
      `Dear ${data.name}`,
    );
  ...
  getAllHireUsRequests: asyncHandler(async (req, res) => {
    const hireUs = await db.hireUs.findMany({
  ...
  trashHireUsRequest: asyncHandler(async (req: _Request, res) => {
    ...
    const user = await db.user.findUnique({
      where: { uid: req.userFromToken?.uid || "" },
    });
