...
export default {
  ...
  deleteFreeLancerRequest: asyncHandler(async (req: _Request, res) => {
    const { id } = req.params;
    const isRequestExist = await db.freeLancersRequest.findUnique({
      where: { id: Number(id) },
      select: { id: true },
    });
  ...
  acceptFreeLancerRequest: asyncHandler(async (req: _Request, res) => {
    ...
    const hashedPassword = (await passwordHasher(
      randomPassword,
      res,
    )) as string;
    ...
    const createdFreelancer = await db.user.create({
      data: {
        username:
          `${generateUsername(isRequestExist.name)}_${generateRandomStrings(4)}`.toLowerCase(),
        email: isRequestExist.email,
  ...
  updateNicheForFreelancer: asyncHandler(async (req: _Request, res) => {
    const { id } = req.params;
    const { niche } = req.body as TFREELANCER;
    await db.nichesForFreelancers.update({
      where: { id: Number(id) },
      data: { niche },
    });
    httpResponse(req, res, SUCCESSCODE, SUCCESSMSG);
  ...
  listSingleNicheForFreelancer: asyncHandler(async (req: _Request, res) => {
    const { id } = req.params;
    const niche = await db.nichesForFreelancers.findUnique({
      where: { id: Number(id) },
    });
  ...
  listSingleFreelancer: asyncHandler(async (req, res) => {
    ...
    if (!username)
      throw { status: BADREQUESTCODE, message: "Username is required" };
