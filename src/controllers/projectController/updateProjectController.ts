...
export default {
  ...
  createInterestedFreelancers: asyncHandler(async (req, res) => {
    ...
    const { interestedFreelancerWhoWantToWorkOnThisProject } =
      req.body as TPROJECT;
    if (!projectSlug)
      throw { status: BADREQUESTCODE, message: "Project slug is required." };
  ...
  removeFreelancerFromInterestedList: asyncHandler(
  ...
  selectFreelancerForProject: asyncHandler(async (req, res) => {
    const { projectSlug } = req.params;
    const { selectedFreelancersForThisProject: freelancerName } =
      req.body as TPROJECT;
    if (!freelancerName)
      throw { status: BADREQUESTCODE, message: "Freelancer name is required." };
    if (!projectSlug)
      throw { status: BADREQUESTCODE, message: "Project slug is required." };
  ...
  updateProgressOfProject: asyncHandler(async (req, res) => {
    ...
    if (!project?.selectedFreelancersForThisProject.includes(freelancerName))
      throw {
        status: BADREQUESTCODE,
        message:
          "You can't update progress of this project since you are not working on this project",
      };
    ...
    if (project?.progressPercentage > progressPercentage)
      throw {
        status: BADREQUESTCODE,
  ...
  changeProjectStatus: asyncHandler(async (req, res) => {
    ...
    if (!projectStatus)
      throw { status: BADREQUESTCODE, message: "Field is required." };
    if (!projectSlug)
      throw { status: BADREQUESTCODE, message: "Project slug is required." };
  ...
  writeReviewAndGiveRating: asyncHandler(async (req, res) => {
    const { projectSlug } = req.params;
    const {
      starsByClientAfterProjectCompletion,
    ...
    const updatedProject = await db.project.update({
      ...
      data: {
        commentByClientAfterProjectCompletion: review,
        starsByClientAfterProjectCompletion: Number(
          starsByClientAfterProjectCompletion,
        ),
      },
      select: {
  ...
  updateProjectBySlug: asyncHandler(async (req, res) => {
    ...
    if (isNaN(deadLineDate.getTime()))
      throw { status: BADREQUESTCODE, message: "Invalid deadline date." };
    ...
    const project = await db.project.update({
      ...
      data: {
        ...updatedData,
        projectSlug: generateSlug(updatedData.title),
        bounty: calculate90Percent(updatedData.bounty),
  ...
  makeProjectOutsource: asyncHandler(async (req, res) => {
    ...
    await db.project.update({
      ...
      data: {
        ...
        bounty: calculate90Percent(project?.bounty || 0),
      },
    });
    httpResponse(req, res, SUCCESSCODE, SUCCESSMSG, { project });
  }),
