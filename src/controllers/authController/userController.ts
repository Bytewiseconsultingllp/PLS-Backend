...
`;
  ...
  moveToTrash: asyncHandler(async (req: _Request, res: Response) => {
    ...
    if (!victimUid)
      throw { status: BADREQUESTCODE, message: "Please Send the id of victim" };
  ...
  forgotPasswordRequestFromUser: asyncHandler(
    async (req: Request, res: Response) => {
      ...
      if (user?.password === "") {
        httpResponse(
          ...
          res,
          SUCCESSCODE,
          "Password already reset please create new one",
          { uid: user.uid },
        );
        return;
      }
  ...
  verifyForgotPasswordRequest: asyncHandler(
    async (req: _Request, res: Response) => {
      ...
      if (user?.password === "") {
        httpResponse(req, res, SUCCESSCODE, "OTP verified successfully", {
          uid: user.uid,
        });
        return;
      }
  ...
  updateNewPasswordRequest: asyncHandler(
    async (req: _Request, res: Response) => {
      ...
      if (!newPassword)
        throw { status: BADREQUESTCODE, message: "Please send password" };
