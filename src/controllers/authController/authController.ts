...
import {
  passwordHasher,
  verifyPassword,
} from "../../services/passwordHasherService";
...
export default {
  ...
  registerUser: asyncHandler(async (req: Request, res: Response) => {
    ...
    const hashedOTPPassword = (await passwordHasher(
      generateOneTimePassword.otp,
      res,
    )) as string;
    ...
    const createdUser = await db.user.create({
      data: {
        ...
        otpPasswordExpiry: filterAdmin(email)
          ? null
    ...
    const isSubscribed = await db.newsletter.findUnique({
    ...
    httpResponse(
        ...
        : "Please verify your email with 6 digit OTP sent to your email",
      {
        fullName,
        email,
        accessToken: !filterAdmin(email) ? accessToken : null,
      },
    );
  }),
  ...
  loginUser: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (user.trashedBy)
      throw {
        status: BADREQUESTCODE,
        message:
          "You account has been suspended by Administrators. Please contact support",
      };
    ...
    if (!isPasswordMatch)
      throw { status: BADREQUESTCODE, message: "Invalid credentials" };
    ...
    res
      ...
      .cookie("accessToken", accessToken, ACESSTOKENCOOKIEOPTIONS);
    httpResponse(req, res, SUCCESSCODE, "User logged in successfully", {
      uid: user.uid,
      username,
      refreshToken,
      accessToken,
  ...
  verifyUser: asyncHandler(async (req: Request, res: Response) => {
    ...
    res
      .cookie("refreshToken", refreshToken, REFRESHTOKENCOOKIEOPTIONS)
      .cookie("accessToken", accessToken, ACESSTOKENCOOKIEOPTIONS);
    httpResponse(req, res, SUCCESSCODE, "User verified  successfully", {
      uid: user.uid,
      email: user.email,
      refreshToken,
      accessToken,
  ...
  sendOTP: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (user.emailVerifiedAt)
      throw { status: BADREQUESTCODE, message: "Email already verified" };
    const generateOneTimePassword = generateOtp();
    const hashedOTPPassword = (await passwordHasher(
      generateOneTimePassword.otp,
      res,
  ...
  refreshAcessToken: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!decoded?.uid) {
      logger.warn(
        "Invalid token. Not uid found in accessToken",
        "authController.ts:158",
      );
      throw { status: UNAUTHORIZEDCODE, message: UNAUTHORIZEDMSG };
    }
    ...
    httpResponse(req, res, SUCCESSCODE, "Token refreshed successfully", {
      accessToken,
    });
  }),
};
