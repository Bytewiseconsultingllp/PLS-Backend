...
const freeLancerControllerV2 = {
  getFreeLancerJoinUsRequest: asyncHandler(async (req: _Request, res) => {
    ...
    const legalAgreementsCreate:
      | Prisma.LegalAgreementsCreateNestedOneWithoutProfileInput
      | undefined = freeLancer.legalAgreements
      ? {
          create: {
                  ...
                  identityVerification: {
                    create: {
                      idType:
                        freeLancer.legalAgreements.identityVerification.idType,
                      taxDocType:
                        freeLancer.legalAgreements.identityVerification
                          .taxDocType,
                      addressVerified:
                        freeLancer.legalAgreements.identityVerification
                          .addressVerified,
    ...
    if (freeLancer.industryExperience) {
      profileData.industryExperience = {
        create: {
          selectedIndustries:
            freeLancer.industryExperience.selectedIndustries || [],
        },
      };
    }
      ...
      profileData.softSkills = {
        create: {
          ...
          communicationFrequency:
            freeLancer.softSkills.communicationFrequency || "",
      ...
      profileData.projectQuoting = {
        create: {
          compensationPreference:
            freeLancer.projectQuoting.compensationPreference || "",
          ...
          willSubmitProposals:
            freeLancer.projectQuoting.willSubmitProposals || "",
        },
      };
    }
  ...
  deleteFreeLancerRequest: asyncHandler(async (req: _Request, res) => {
    ...
    if (!id)
      throw { status: BADREQUESTCODE, message: "Profile id is required." };
    const isRequestExist = await db.profile.findUnique({
      where: { id: id },
      select: { id: true },
    });
  ...
  acceptFreeLancerRequest: asyncHandler(async (req: _Request, res) => {
    ...
    const createdFreelancer = await db.user.create({
      data: {
        username:
          `${generateUsername(profile.whoYouAre.fullName || "freelancer")}_${generateRandomStrings(4)}`.toLowerCase(),
