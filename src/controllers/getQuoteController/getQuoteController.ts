...
export default {
  ...
  createQuote: asyncHandler(async (req, res) => {
    const { name, email, phone, company, address, deadline, detail, services } =
      req.body as TGETQUOTE;
  ...
  deleteServicesForQuote: asyncHandler(async (req, res) => {
    ...
    const checkIfServiceExist = await db.createServicesForQuote.findUnique({
      where: { id: Number(id) },
    });
  ...
  getSingleQuote: asyncHandler(async (req, res) => {
    const quote = await db.getQuote.findUnique({
  ...
  getAllQuote: asyncHandler(async (req, res) => {
    const quotes = await db.getQuote.findMany({
  ...
  deleteQuote: asyncHandler(async (req: _Request, res) => {
    const quote = await db.getQuote.delete({
      where: { id: Number(req.params.id) },
    });
    httpResponse(req, res, SUCCESSCODE, SUCCESSMSG, quote);
  }),
